...
export default class Pacman
{
	...
	constructor (x, y)
	{
		...
		this.animMove = new Anim(r.pacmanMove, this.width, 12);
		...
		this.animDead = new Anim(r.pacmanDead, this.width, 12);
	...
	draw(g, world)
	{
		...
		/*let [wall, area, side] = world.checkWall(this.x, this.y, this.width, this.height);
		if (wall != null) {
			if (side == 'LEFT') g.fillStyle = 'red';
			else if (side == 'RIGHT') g.fillStyle = 'magenta';
			else if (side == 'UP') g.fillStyle = 'green';
			else if (side == 'DOWN') g.fillStyle = 'white';
	...
	update (world)
	{
		...
		if (side == 'LEFT') this.x = wall.x - this.width;
		else if (side == 'RIGHT') this.x = wall.x + wall.width;
		else if (side == 'UP') this.y = wall.y - this.height;
		else if (side == 'DOWN') this.y = wall.y + wall.height;
	}
};
